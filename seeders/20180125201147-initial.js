module.exports = {
  up: (queryInterface, Sequelize) => {
    ...
    return Promise.all([ queryInterface.bulkInsert('Years', yearsData), queryInterface.bulkInsert('Reviewers', reviewersData) ]).then(() => queryInterface.sequelize.query('SELECT * FROM "Years" ORDER BY id ASC').then(yearsResult => queryInterface.sequelize.query('SELECT * FROM "Reviewers" ORDER BY id ASC').then((reviewersResult) => {
      const years = yearsResult[0];
      const reviewers = reviewersResult[0];

      const getReviewer = name => reviewers.filter(reviewer => reviewer.name.toLowerCase() === name.toLowerCase())[0];
      const getYear = name => years.filter(year => year.name.toLowerCase() === name.toLowerCase())[0];
  ...
  down: (queryInterface, Sequelize) => queryInterface.bulkDelete('TopMovies', null, {}).then(() => queryInterface.bulkDelete('ViewStats', null, {}).then(() => queryInterface.bulkDelete('Reviewers', null, {}).then(() => queryInterface.bulkDelete('Years', null, {})))),
};
